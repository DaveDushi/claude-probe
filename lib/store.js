      ...
      try {
        ...
        for (const line of lines) {
          try {
            ...
            // Capture first user prompt or text content as preview
            if (!preview && evt.kind === 'user_message' && evt.text) {
              preview = evt.text.slice(0, 150).trim();
            }
            ...
            if (evt.kind === 'session_end') {
              ...
              ended = true;
              // Prefer session result as preview if available
              const trimmedResult = (evt.result || '').slice(0, 150).trim();
              if (trimmedResult) {
